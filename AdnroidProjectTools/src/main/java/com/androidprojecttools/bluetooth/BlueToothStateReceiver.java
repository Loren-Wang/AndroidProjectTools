...
public class BlueToothStateReceiver extends BroadcastReceiver {
    ...
    public BlueToothStateReceiver(@NonNull BlueToothReceiverCallback blueToothReceiverCallback) {
        if(blueToothReceiverCallback == null){
            throw new NullPointerException(Setting.APPLICATION_CONTEXT.getResources().getString(R.string.broadcast_receiver_hint_callback_is_not_null));
        }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (action){
            ...
            case ACTION_BOND_STATE_CHANGED://远程蓝牙设备状态改变的时候发出这个广播, 例如设备被匹配, 或者解除配对
                LogUtils.logD(TAG,"远程蓝牙设备状态改变");
                break;
            case ACTION_PAIRING_REQUEST://配对时，发起连接
                LogUtils.logD(TAG,"发起蓝牙配对请求");
                break;
            case ACTION_CLASS_CHANGED://一个远程设备的绑定状态发生改变时发出广播
//                LogUtils.logD(TAG,"远程蓝牙设备的绑定状态发生改变");
                break;
